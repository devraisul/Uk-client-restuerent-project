...
import { editRestaurent, editRestaurentLogo, getRestaurent } from '../../Apis/Restaurent';
...
const UpdateRestaurent = () => {
  ...
  const onSubmit = async (e) => {
    ...
    if (formImageUrl) {
      editRestaurent(user.restaurant[0]?.id, formData).then(res => {
        Data.append('logo', formImageUrl, formImageUrl.name);
        editRestaurentLogo(user.restaurant[0].id, Data)
          .then(res => {
            console.log({ res });
            ...
            <form style={{ width: '500px' }} onSubmit={(e) => onSubmit(e)}>
              ...
              <div className={styles.imageUpDropdownContainer}>
                ...
                <input
                  ...
                  onChange={(e) => {
                    onFileChange(e)
                    setPlaceholderImage(URL.createObjectURL(e.target.files[0]));
                  }} />
              </div>
              {/* NAME  */}
